...
import json
import re
player_number_pattern = re.complie(r"[pP]?(\d)")
max_pattern = re.compile(r"max:\s*(\w)")
min_pattern = re.compile(r"min:\s*(\w)")
...
class Game(models.Model):
	...
	current_block_instance = models.ForeignKey(BlockInstance, blank=True, null=True, on_delete=models.SET_NULL, editable=False)
	...
	def start(self):
		...
		self.save()
		new_cycle = Cycle(block_instance=self.current_block_instance, game=self)
		new_cycle.save()
	...
	def latest_cycle(self):
		return Cycle.objects.filter(game=self).order_by('-timestamp')[0]
	...
	def total_cycles(self):
		return Cycle.objects.filter(game=self).count()

	def current_block_cycles(self):
		return Cycle.objects.filter(game=self, block_instance=self.current_block_instance).count()
	...
	def deepCopy(self, name=None):
		while True:
			try:
				...
				pass
		newGame = Game(
			name = (name if name != None else self.name + " Copy"),
			slug = slug,
			player_count = self.player_count,
			previous_cycle_displayed = self.previous_cycle_displayed,
			current_cycle_displayed = self.current_cycle_displayed,
			enforce_choice_order = self.enforce_choice_order,
			chat = self.chat,
			show_others_points = self.show_others_points,
			starting_block = self.starting_block,
			instructions = self.instructions,
			mute_feedback = self.mute_feedback,
			background = self.background,
			)
		newGame.save()
		newGame.points.add(*self.points.all())
		newGame.choices.add(*self.choices.all())
		newGame.save()
		return newGame.pk
...
class BlockEndRule(models.Model):
	block = models.ForeignKey(Block, on_delete=models.CASCADE)
	min_cycles = models.PositiveSmallIntegerField(verbose_name="Minimum number of cycles before checking, if different from condition's minimum cycles", default=0, blank=True)
	...
	next_block = models.ForeignKey(Block, on_delete=models.CASCADE, null=True, blank=True, verbose_name="Next Condition - Leave blank to repeat this block or end the game")
	...
	def return_dict(self, max_point_seat = None):
	...
	def rule_met(self, block_instance):
		...
		if self.max_cycles > 0 and block_instance.elapsed_cycles >= self.max_cycles:
			return self.return_dict(block_instance)
		...
		for point_instance in point_instances:
			if self.point_max_value and point_instance.value > self.point_max_value:
				...
				return self.return_dict(point_instance.seat_number)
...
class BlockPlayerInstructions(models.Model):
	block = models.ForeignKey(Block, on_delete=models.CASCADE)
...
class RuleInstance(models.Model):
	...
	def feedback(self, game):
		if self.single_rule:
			return self.single_rule.feedback(game)
		if self.combo_rule:
			return self.combo_rule.feedback(game)
		return "ERROR"
	...
	def compare_choices(self, choices):
		if self.combo_rule:
			return self.combo_rule.compare_choices(choices)
		return False

class SingleRule(models.Model):
	block = models.ForeignKey(Block, on_delete=models.CASCADE)
...
class SingleRuleModification(models.Model):
...
class ComboRuleModification(models.Model):
...
class Cycle(models.Model):
	...
	def is_complete(self):
		if self.complete:
			return True
		if self.move_set.count() == self.game.player_count:
			self.complete=True
	...
	def pattern(self):
		...
		for move in self.move_set.all().order_by("seat_number"):
			result[move.seat_number] = move.choice.pk
		return result.join(",")

	def max_point_seat(self, point):
		if not self.point_log:
			return None
		point_log = json.loads(self.point_log)
		try:
	...
	def process(self):
		...
		for move in self.move_set.all():
			...
			for rule_instance in RuleInstance.objects.filter(block_instance=self.block_instance, choice=move.choice, single_rule__isnull=False):
				...
				results[point_instances.name()].append((rule_instance.num_points, move.seat_number, rule_instance.feedback(self.block_instance.game)))
		...
		for rule_instance in RuleInstance.objects.filter(block_instance=self.block_instance, combo_rule__isnull=False):
			if rule_instance.compare_choices(choices):
				...
				for point_instance in point_instances:
					if not point_instance.name() in results.keys():
						results[point_instance.name()] = []
					results[point_instance.name()].append((rule_instance.num_points, "GROUP", rule_instance.feedback(self.block_instance.game)))
					point_instance.value += rule_instance.num_points
					point_instance.save()
		...
		for point_instance in PointInstances.objects.filter(game=self.block_instance.game):
		...
		if end_dict:
			...
			if end_dict['destination'] == None:
				...
				self.save()
				return results
			next_block_instance = self.game.next_block(end_dict['destination'], end_dict['hard_refresh'])
			if end_dict['hard_refresh']:
				for modification in end_dict['single_rule_modifications']:
					modification.apply(self.game)
				for modification in end_dict['combo_rule_modifications']:
					modification.apply(self.game)
			new_cycle = Cycle(block=next_block_instance, game=self.block_instance.game)
		else:
			new_cycle = Cycle(block=self.block, game=self.block_instance.game)
		new_cycle.save()
		return results
