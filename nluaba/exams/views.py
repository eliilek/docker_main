...
def maintenance_probe(request):
	...
	return render(request, 'exams/maintenance_probe.html', {'courses':courses})

def maintenance_probe_select(request):
	instances = MaintenanceInstance.objects.filter(user=request.user, finished__isnull=True)
	if request.user.is_superuser:
		return render(request, 'exams/maintenance_probe_select.html', {"superuser":True})
...
def maintenance_create(request):
	...
	for key in request.POST:
		if "checkbox-" in key:
			course = Course.objects.get(pk=request.POST[key])
			courses.append(course)
	existing = Maintenance.objects.annotate(c=Count('courses')).filter(c=len(courses))
	if existing.count() != 0:
		for course in courses:
			existing = existing.filter(courses=course)
	if existing.count() != 0:
		maintenance = existing[0]
	else:
...
def maintenance_take(request, instance_pk, question_index=None):
	try:
		maintenance_instance = MaintenanceInstance.objects.get(pk=instance_pk)
	except:
		return render(request, 'exams/plain.html', {'msg':"I couldn't find the test you're looking for.\nUse the above link to return to the menu."})
	if request.user != maintenance_instance.user:
		return render(request, 'exams/plain.html', {'msg':"You have reached this page in error.\nUse the above link to return to the menu."})
	questions = maintenance_instance.maintenanceresponse_set.all().order_by('id')
	if question_index != None:
		try:
			question = questions[question_index].question
		except Exception as e:
			print(e)
			return maintenance_take(request, instance_pk, None)
	else:
		unanswered = maintenance_instance.maintenanceresponse_set.filter(answer="").order_by('id')
	...
	to_send['name'] = "Maintenance Probe"
	to_send['answers'] = answers
	if question.image:
		to_send['image_url'] = cloudinary.CloudinaryImage(question.image.image.public_id).build_url(width=500)
	to_send['maintenance_id'] = maintenance_instance.id
	to_send['total_question_number'] = maintenance_instance.total_questions()
...
def submit_maintenance_response(request, instance_pk):
	try:
		instance = MaintenanceInstance.objects.get(pk=instance_pk)
	except:
		return render(request, 'exams/plain.html', {'msg':"I couldn't find the test you're looking for.\nUse the above link to return to the menu."})
	if request.method != "POST":
	...
	unanswered_questions = instance.maintenanceresponse_set.filter(answer="").order_by('id')
	if unanswered_questions.count() == 0:
		instance.finished = timezone.now()
		instance.save()
		return redirect('exams:view_maintenance_instance_data', instance.pk)
	...
	return redirect('exams:maintenance_take', instance_pk=instance_pk, question_index=next_question)
	...
	if 'instance' in request.session:
		...
		if request.method == "POST":
			...
			if test.time_limit:
				...
				test_instance.elapsed = old_time + additional_time
				test_instance.save()
...
def select_maintenance_student(request):
	if not request.user.is_superuser:
		return render(request, "exams/plain.html", {"msg":"You are not authorized to view this page.\nUse the above link to return to the menu."})
	return render(request, "exams/maintenance_student_select.html", {"students":get_user_model().objects.filter(maintenanceinstance__isnull=False).distinct().order_by('name')})

def view_maintenance_data(request, student_pk = None):
	if request.user.is_superuser and student_pk == None:
		instances = MaintenanceInstance.objects.all().order_by('created')
	elif request.user.is_superuser:
		try:
			student = get_user_model().objects.get(pk=student_pk)
		except:
		...
		instances = MaintenanceInstance.objects.filter(user=request.user).order_by('created')
	paginator = Paginator(instances, 25)
	page = paginator.get_page(request.GET.get('page', 1))
	clean_instances = []
	return render(request, "exams/view_maintenance.html", {"instances":page, "super":request.user.is_superuser})

def view_maintenance_instance_data(request, instance):
	try:
		instance = MaintenanceInstance.objects.get(pk=instance)
	except:
...
def download_maintenance_student_select(request):
	if not request.user.is_superuser:
		return render(request, "exams/plain.html", {"msg":"You are not authorized to view this page.\nPlease login as a superuser or use the above link to return to the menu."})
	students = [user for user in get_user_model().objects.filter(maintenanceinstance__isnull=False).distinct()]
	return render(request, "exams/download_maintenance_select.html", {"students":students})

def download_maintenance_course_select(request):
	if not request.user.is_superuser:
		return render(request, "exams/plain.html", {"msg":"You are not authorized to view this page.\nPlease login as a superuser or use the above link to return to the menu."})
	return render(request, "exams/download_maintenance_select.html", {"courses":Course.objects.all()})

def download_maintenance_student(request, student):
	if not request.user.is_superuser:
		return render(request, "exams/plain.html", {"msg":"You are not authorized to view this page.\nPlease login as a superuser or use the above link to return to the menu."})
	try:
		student = get_user_model().objects.get(pk=student)
...
def download_maintenance_course(request):
	if not request.user.is_superuser:
		return render(request, "exams/plain.html", {"msg":"You are not authorized to view this page.\nPlease login as a superuser or use the above link to return to the menu."})
	if request.method != "POST":
		return render(request, 'exams/plain.html', {'msg':"You have reached this page in error.\nUse the above link to return to the menu."})
	...
	for key in request.POST:
		if "checkbox-" in key:
			course_pks.append(request.POST[key])
	args_dict = {}
...
def delete_maintenance_instance(request, instance):
	try:
		instance = MaintenanceInstance.objects.get(pk=instance)
	except:
		return render(request, "exams/plain.html", {"msg":"I couldn't find the maintenance instance you're looking for.\nUse the above link to return to the menu."})
	instance.delete()
	return redirect("exams:view_maintenance_data")
