...
def splash(request):
	return render(request, 'safmeds/splash.html', {'title':"NLU ABA", 'assessment':(request.user.assessment_set.count() != 0)})
...
def first_login(request):
	if request.method == 'POST':
		...
		try:
			calibration = Deck.objects.get(name="Calibration Deck")
		except:
			calibration = None
		try:
			orientation = realexams.models.Test.objects.get(name="Orientation Quiz")
		except:
			orientation = None
		if password != form.data['password2']:
		...
		if existing.count() != 0 and existing[0].has_usable_password():
		...
		elif existing.count() != 0 and existing.filter(name__iexact=name).count() != 0:
			user = existing[0]
			user.set_password(password)
			user.save()
			login(request, user)
			if calibration:
				calibration.students.add(user)
				calibration.save()
			if orientation:
				orientation.students.add(user)
				orientation.save()
			return splash(request)
		elif existing.count() != 0:
			return render(request, 'registration/signup.html', {'form':form, 'errormsg':"This e-mail address is already in use."})
		user = get_user_model()(name=name, email=email)
		user.save()
		user.set_password(password)
		user.save()
		login(request, user)
...
def report(request):
	...
	print(request.POST)
	args_dict = {'responses':json.loads(request.POST['responses']), 'student':request.user}
	args_dict['deck'] = request.POST['deck']
	args_dict['practice_type'] = request.POST['practice_type']
...
def progress(request, deck):
	...
	if request.user.is_superuser:
		...
		paginator = Paginator(practices_list, 30)
